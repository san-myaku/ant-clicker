...
- **Dev mode** (long-press 描画設定 5s, or `?dev=1`) → `_activateDevMode()`: reveals `#dev-tools` + `#dev-raid-tools`, unlocks research, sets `S._devMode`. While `S._devMode`, `render()` draws a live FPS readout (rolling `S.perf.fps`) top-right in screen space (green ≥55 / amber ≥30 / red below).
