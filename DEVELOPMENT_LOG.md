...
- `drawRoomInteriorRealism()` を追加し、部屋内に天井側の崩れ影、床側の踏み固めハイライト、細い擦れ跡、天井側の崩れ粒を静的土レイヤーへ焼くようにした。
- `drawRoomRimChips()` を追加し、部屋リムに決定的乱数ベースの欠け/黒ずみを重ねた。部屋ジオメトリやセーブデータは変更せず、見た目だけで「掘られた空洞」の縁を強める。
- 欠け表現は `drawRoomSoilLip()` 側に集約し、内側ディテールとリム描画で二重に乗らないようにした。
...
- `generateMeshNest({ growth:true })` を追加。通常の `generateMeshNest()` は従来どおり全ノード/全エッジ開通済みのプレビュー、growth指定時だけ `S._meshGrowth=true` とし、入口＋女王房と入口-女王エッジのみ開通済みにする。バンド状態も第0層の実測範囲から開始する。
- devツールの「巣生成(試作):」行に `#btn-dev-mesh-growth`（🌱成長メッシュ）を追加。`window._dbg.generateMeshNest({growth:true})` からも起動できる。
- `getDigTarget()` はメッシュ成長中だけ、事前生成済みの未可視ノードへ伸びる未完成エッジを掘削候補にする。旧 `expandMap()` / `forceExpandRoom()` による木構造の追加生成は growth 中だけ停止し、メッシュ外の部屋が混ざらないようにした。
- 両端が可視済みの未開通ループエッジも `mesh-loop` ターゲットとして掘れるようにした。これで部屋の解放後に残るループ通路も建築AIが完成できる。
- devメッシュは `S._devTransientWorld=true` とし、自動保存と通常 `G.save()` をブロックする保存ガードを追加。試作巣を触っても通常セーブを上書きしない。
- 検証: `git diff --check` と inline script の `node --check` は成功。Playwright（file URL `?dev=1`）で `generateMeshNest({growth:true})` を実行し、80ノード/88エッジ、初期可視2ノード/開通1エッジから開始、`updateBuilderLogic()` 進行後もノード数80のまま可視9ノード/開通8エッジへ進むことを確認。可視済み同士の未開通ループは `mesh-loop` として選択され、`G.save({showToast:false})` 後も localStorage 保存は発生せず。コンソール/pageerror 0件。目視用スクショ `qa_screenshots/mesh_f4_growth_smoke.png` を保存。

## 2026-06-18 メッシュ巣F3: ループ対応の重み付き経路探索 + 食料を種色へ

`NEST_STRUCTURE_REDESIGN_PLAN.md` の F3 に着手。既存タスク（`go_q`/`go_n`/採餌/給餌/兵隊移動など）はすべて `getPath()` を使っていたため、呼び出し側は壊さずに経路探索本体を差し替えた。

- `findNodePath()` / `getPath()` を BFS から重み付き探索へ変更。完成済みトンネル（`edgeFrac >= 0.99`）だけを通り、ベジェ長ベースの距離に、現在歩行中/予約中のエッジ混雑と小さなランダム揺らぎを加味する。
- 混雑はフレーム単位で `S._pathCongestion` にキャッシュし、`getPath()` が返したルートの先頭数エッジも短期予約として加算する。これにより同じフレームで複数アリが同方向へ割り当てられても、次のアリが別ルートを選びやすい。
- 既存の `move()`、移動タスク、`getPathViaEntrance()` の API は維持。メッシュ巣のループでは距離/混雑に応じて複数経路を使い、旧木構造の本番巣でもそのまま動く。
- デバッグ用に `window._dbg.findNodePath` / `getPath` / `getPathDebugStats` を公開。キーPのパス表示文言も「最短」から「現在選ばれる経路」に更新。
- F2軽仕上げ: 室内在庫の食料粒を緑の丸から黄土〜琥珀寄りの種粒（影・筋・明るい縁取り/ハイライト付き）へ変更。建築アリの土と混ざらないよう、暗い茶色ではなく食料だけ明るい種色にしている。持ち運び中の食料やUI色は資源識別として緑のまま。
- 検証: `git diff --check` と inline script の `node --check` は成功。Playwright（file URL `?dev=1`）で `generateMeshNest()` を実行し、83ノード/92エッジの生成、30/30 経路成功、経路予約が59本のエッジへ分散、コンソール/pageerror 0件を確認。モーダルを消した目視用スクショ `qa_screenshots/mesh_f3_smoke_clean.png` では79ノード/87エッジ、食料入り部屋27、室内食料の種色表示を確認。
- 残作業: F4はdev成長モードとして接続開始済み。F5（meshを本番生成・新セーブ形式へ採用、devトグル撤去）は未着手。通常の `initWorld` / `expandMap` と既存セーブ形式はまだ維持。

## 2026-06-17 巣の構造リライト(F1): メッシュ型ジェネレーターを devトグルで試作

`NEST_STRUCTURE_REDESIGN_PLAN.md` のフェーズ分けに沿って着手。木構造（1本の主竪坑＋枝）をやめ、ループのあるネットワーク（メッシュ）に作り直す本丸の最初のフェーズ。本番の生成・セーブには未接続（devトグル限定のプレビュー）。

- **`generateMeshNest()`** を追加（`initWorld()` の直後）。`MESH_GEN_DEFAULTS` でバンド数・1バンドあたりのチャンバー数・降り口本数などを調整可能（決め打ちしない設計方針どおり、実機で詰める前提のパラメータ化）。
  - 入口1つ→女王の間→**入口直下で複数の降り口（`descents`本）に分岐**→各深度バンドに密なチャンバー・クラスターを配置。
  - 層内リンク（`intraLinks`）と層間リンク（`interLinksPerBand`、最近傍以外も複数本）でループを生成。スパニングツリーを上回るエッジ＝サイクルが必ず生まれる構成。
  - チャンバーは種別ごとに `meshLensShape()` でレンズ/涙型（横長・床フラット・天井ドーム）のblobパラメータを割り当て、`meshRoomRadius()` で種別ごとのサイズ差（女王＝大、ゴミ＝小 等）を付与。
  - 既存の描画一式（`buildNodeBlobAndSlots`/`buildEdgeStamps`/静的キャッシュ等）・バンド状態(`S.band`)・blob生成をそのまま流用。生成後は全ノード/エッジを可視化し、カメラを再フィット。
  - **初期バグ修正**: 1バンド内を2行に詰める際、行間隔を `bandHeight` 全体に引き伸ばしていたため「層内の行間」が「層間の隙間」より広くなり、層がレイヤーとして読めない問題があった。行間隔をチャンバー半径基準（`rowStep = maxRad*2.0`）に変更して解消。
- **devツール**: 「巣生成(試作):」行に🕸️新メッシュ巣ボタンを追加(`btn-dev-mesh-nest`)。`_activateDevMode()` 配下で配線。`window._dbg.generateMeshNest` でコンソールからも呼べる。
- **副次修正（既存バグ）**: `?dev=1` で起動すると、スクリプト末尾で定義される `const TOAST` の初期化前に `unlockResearchSystem()` 経由で `toast()` が呼ばれ `ReferenceError`（TDZ）でクラッシュし、`S.full` が一切作られない不具合があった（本タスクの新ジェネレーター検証に必須の経路だったため修正）。`_activateDevMode(false)` の呼び出しを `setTimeout(...,0)` で次ティックへ遅延し解消。
- 検証: 実ブラウザ（Playwright経由でheadless Chromium、`localhost:8765`）で確認。
  - 新メッシュ生成→コンソール/pageerror 0件。25回連続生成しても例外なし（ノード43〜49・エッジ44〜60で安定）。
  - 生成後にBFSで連結性確認: 全ノード到達可能、スパニングツリー(n-1本)を上回るエッジが複数本＝ループが実在することを確認(例: 46ノード/51エッジ→スパニングツリー超過6本)。
  - 生成後にワーカーアリを追加し8秒間実時間で通常ループ(`move`/AI/描画)を走らせてもエラー0。新トポロジーでも蟻が普通に移動できることを確認。
  - スクショで、女王の間直下から複数の降り口→深度ごとに密集したクラスター→層内/層間ともループ状の梯子構造、椀状陰影・トンネル壁陰影など既存描画がそのまま乗ることを確認。
- 当時の残作業: F2（薄壁の密着・トンネル短縮・ラッパ口などの見た目の詰め）、F3（蟻の経路探索のループ対応＝最短/混雑回避）、F4（建築AI・成長ロジックの新トポロジー対応）、F5（セーブ新形式への正式採用とdevトグル撤去）。現状は旧ジェネレーター(`expandMap`/`initWorld`)が本番のまま完全に維持されている。

### 追記: 横方向の配置も密集化（同日、ユーザーの実機確認後のフィードバックで修正）

実機（スマホ、`raw.githack.com` 経由でブランチ参照）で確認したところ、チャンバー間のトンネルが想定より長く見えるとの指摘。原因は行内の列配置で、各行をチャンバーの実サイズに関係なくワールド幅いっぱい(`usableW`)に均等分散していたため（縦方向の行間隔は前回の修正で半径基準に直したが、横方向の列間隔は同じ「全体に引き伸ばす」バグが残っていた）。

- 列間隔を `colStep = maxRad * 2.6`（半径基準。レンズ形状が横長＝`blobScaleX`最大1.45のため縦の`rowStep`より広めに）に変更し、各行を入口直下の中心(`cx`)を軸に詰めて配置（奇数行は`colStep*0.5`ずらしてジグザグに密集）。
- 効果（Playwright計測、`localhost:8765`）: エッジ長の中央値が約363→約126〜136(dp単位)に短縮（旧の1/3弱）。15回連続生成でも例外0、median 122〜136で安定。
- 副次効果: チャンバーが近接した分、層内リンクの距離閾値(`dp(440)`)に収まる近傍ペアが増え、ループ数が増加（1回の生成例: 47ノード/106エッジ→スパニングツリー超過60本＝ループ60個）。全ノード到達可能も確認。
- ワーカーアリ追加→8秒間のライブ実行でもコンソール/pageerrorエラー0、`go_in`/`ret`タスクで正常に移動。
- スクショで、密に詰まったクラスター同士が短い枝でつながり、層内・層間ともループが視認できる構造を確認。

### 追記: 縦方向（バンド間隔）も密集化（同日、別セッション引き継ぎで修正）

横の密集化後も実機で「上のクラスターの下に長い縦トンネルだけの空白があり、層が離れて見える」問題が残っていた。原因は `bandHeight: dp(520)` が大きすぎたこと。1クラスターの実高さは約135pxなのに1バンドに520px割り当てていたため、各クラスターの下に約370px＋`bandGap`150px＝約520pxの空の土が生まれ、バンド間が約835px間隔（中心間）に広がっていた（縦トンネルだけが長く伸びる＝動線も単調）。

- `bandHeight: dp(520)→dp(160)`、`bandGap: dp(150)→dp(50)` に変更。バンド中心間隔が約835px→約210pxになり、クラスター（約120px）間の土の床が約147pxの薄いフロアとして積層。全体の縦範囲も y586〜4907 → y590〜2032 に圧縮。
- 検証（`localhost:8765`、`window._dbg.generateMeshNest({bandHeight:dp(160),bandGap:dp(50)})` でライブ確認後に既定値へ同値ベイク）: 6バンドが密に積層し、層内・層間とも複数トンネルでループしながらつながる構造をスクショで確認（44ノード/100エッジ＝ループ多数）。コンソールエラー0。

### 追記: 配置を「整列した行」→「ジッター密集パック」に作り直し（同日、ユーザーフィードバック）

実機で「層と層の間が長すぎる／部屋が重なる／有機的でなく、部屋を短いシャフトで繋いだグラフに見える」との指摘。原因は (1) 行ベース配置＋バンドに広い縦割当で層が離れる、(2) 衝突判定が同バンド内のみ＆等方0.65係数で横長レンズに緩く重なる、(3) 部屋が離れる分トンネルが長く目立つ。

- 配置アルゴリズムを刷新: 行配置を廃し、各バンドの薄いスラブ(`slabH=maxRad*2.0`)内に**ジッター配置**（cx周りに`clusterHalfW`で横も密集）。バンドは`slabH+bandGap`で**直下に積層**（`bandGap`を小さく）。
- **異方性の重なり判定** `overlapsAny()` を追加（レンズの横長を考慮：横`lensHX`は種別で1.15〜1.45、縦`lensHY`は0.66）。**全バンド横断**(`allRooms`)で楕円距離<1を弾き、隣バンドとの重なりも解消。`thinWall`は壁の薄さ。48回試行で空き探索、入りきらない時のみ薄い重なりを許容して部屋を失わない。
- `S.band` 状態・ワールド高さ `H` を、固定の`bandHeight`計算から**積層後の実測** `deepestY`/`lastBandTop`/`bandSpacing` ベースに変更。`bandHeight` はレガシー（未使用）。
- 既定値: `bandGap dp(50)→dp(10)`、`thinWall dp(14)→dp(6)`、`interLinksPerBand 3→2`（交差チューブを減らす）。
- 検証（`localhost:8765`）: 縦スパン約1442px→**約718px**に圧縮、密な涙型クラスターに。45〜46ノード/97〜99エッジ、コンソールエラー0。スクショで重なり解消＆密集を確認。残課題: メッシュ＝ループのため交差トンネルがまだ見える（さらに密着させチューブを埋もれさせるか、ループ数を減らすかはユーザーと方向確認中）。

### 追記: トンネルをMST化してクラッタ削減＋曲率で有機化（同日、ユーザーが方向A=有機優先を選択）

「ゴチャつきすぎ。交差トンネルを減らして有機的に。シャフトの曲率や途中の二股もOK」との方向。リンク方式を「各部屋を最近傍N個」→**最小全域木(MST)＋ループ少数**に置換。

- **MST（Prim法・距離）**で全部屋＋女王をつなぐ幹を生成。ユークリッドMSTは平面上で辺が交差しない性質があり、枝分かれ（二股・三股）が自然に出る＝有機的。降り口の中継シャフト生成は廃止（女王の枝分かれはMSTが担当）。
- **ループは少数だけ**: 隣接（≤dp(200)）かつ既存辺と交差しない（生成中は`S.full`未確定なのでローカルな`segIntersect`直線判定）ペアを距離順に `extraLoops` 本（既定4）だけ追加。
- **曲率を有機化**: mesh側の `addE` のみ `edgeCurviness×1.4`（共有関数は不変）。
- 既定値に `extraLoops: 4` 追加。`intraLinks`/`interLinksPerBand`/`descents` はレガシー化（MSTで未使用）。
- 検証（`localhost:8765`）: エッジ 97→**44**（MST40＋ループ4）に半減、交差トンネルが消えて密な涙型クラスターが短い枝でつながる構造に。BFSで全41ノード到達可能、degree≥3のフォーク14箇所＝枝分かれ実在。コンソールエラー0。

### 追記: 詰めすぎ由来の不具合修正（同日）

実機で「部屋が繋がってない所がある／層がどうなってるか分からない」との指摘。診断で原因特定: (1) 詰めすぎでバンドのスラブが部屋数に対し狭く、48回試行が失敗してフォールバックで重ねて配置→**重なり11ペア**、(2) 密着でMST末端(葉9個)の短いトンネルが部屋に埋もれ繋がって見えない、(3) バンド間が約20pxで層が混ざる。

- スラブ `maxRad*2.0→2.8`、横の密集幅 `count*maxRad*0.95→1.15`（部屋数ぶんの面積を確保）。
- フォールバックを「ランダム配置」→**最小重なり位置**（56回試行で最も重なりの少ない候補を保持して使用）。`overlapsAny`(bool)→`overlapPenalty`(重なり数)。
- 既定値 `thinWall dp(6)→dp(18)`（重なり防止＆トンネルが見える）、`bandGap dp(10)→dp(45)`（層がフロアとして読める）。
- 検証（`localhost:8765`）: 重なり **11→1ペア**、層間 約69〜105px に分離して読めるように、トンネルも可視化。45ノード/48エッジ、コンソールエラー0。トレードオフ（密な塊感 vs 明瞭さ）の最終バランスはユーザーの実機判断待ち。

### 追記: ユーザーの手描きスケッチに合わせて「蛇行シャフトの数珠つなぎクラスター」へ（同日）

ユーザーが手描きスケッチを共有（女王＝上の大きい丸、その下に部屋クラスター、部屋同士をうねうね曲がる線＝シャフトで数珠つなぎ、一部ループ、これで1層）。私の事前解釈（ハブ＋放射スポーク）は誤りと判明し、修正。

- 接続を「グローバルMST」→**層内MST（バンドごとに部屋同士を最近傍でつなぐ枝分かれ）＋層間は最短の非交差ペアを `interLayerLinks`(既定2)本だけ縦シャフトで**に変更。女王→第0層も2本。層内ループは `extraLoops`(4)本（一部の部屋が2隣とつながる）。
- シャフトを大きく蛇行: mesh側 `addE` の `curviness` を `max(edgeCurviness×1.4, 0.4)`＋`maxOffsetFactor 0.42`（共有関数は不変）。
- 密度を中間に: `thinWall dp(18)→dp(12)`、`bandGap dp(45)→dp(35)`（クラスター寄りだが蛇行シャフトは見える）。
- 検証（`localhost:8765`）: 42ノード/51エッジ、全ノード連結、重なり**0**、コンソールエラー0。女王（大）が上→蛇行シャフト→部屋の数珠つなぎクラスター、というスケッチ準拠の構造をスクショで確認。ユーザー実機確認待ち。

### 追記: 横幅活用・女王シャフト短縮・蛇行強化（同日、実機フィードバック）

「各層が画面横幅を活かしてない（左右が空く）／女王シャフトが長すぎ／なるべく直線シャフトを無くしたい／二又はOK」との指摘。原因: クラスター幅が `count*maxRad*1.15≈546dp`（世界幅 dp(1800)=2250px の約30%）しか使っていなかった。

- 横幅をパラメータ化: `clusterHalfW = (W/2-margin) * widthUse`（`widthUse` 既定0.72）。部屋数 `roomsPerBand 7→11` で横を埋めて枝を短く保つ。
- 女王シャフト短縮: `firstBandTop dp(280)→dp(190)`（女王の真下に第0層を寄せる。connectGroupsが最短ペアを選ぶので真下の短い縦シャフトになる。女王→第0層は2本＝二又）。
- 蛇行強化（直線を減らす）: `curviness max(×1.5, 0.52)`＋`maxOffsetFactor 0.5`。
- 検証（`localhost:8765`）: 横方向の使用が約30%→**約70%**に。65ノード/74エッジ、全連結、重なり2、コンソールエラー0。女王から短い蛇行シャフト→横幅いっぱいの数珠つなぎクラスターをスクショで確認。部屋の形状は今後調整予定（ユーザー）。

### 追記: シャフトの「途中でY字」分岐（同日）

「二股は部屋のところではなく、シャフトの途中でY字に割れてほしい」との指摘。中継ジャンクション方式を追加。

- `forkConnect(srcId, targetIds)`: src から中継シャフト(`type:'shaft'`, dp(9)) を1本伸ばし、その分岐点から複数部屋へ枝分かれ＝通路の途中でY字。`nearestIn()` で近い部屋を選ぶ。
- 女王→第0層を `forkConnect`（真下に1本→Y字で2部屋）に。層間も各バンドで「下層中心に最も近い上層部屋」を起点に下層の近い部屋へ `forkConnect`。
- 検証（`localhost:8765`）: shaft型ジャンクション6個（女王1＋層間5）すべて degree≥3＝Y字成立。76ノード/85エッジ、全連結、コンソールエラー0。女王の降りシャフトがY字に割れる様子をスクショで確認。

## 2026-06-18 メッシュ巣F2（部屋の形）: 涙型・ラッパ口・有機的な不規則輪郭

F2着手。ユーザー選択の3方向（涙型・床フラット強調／ラッパ口／有機的な不規則輪郭。サイズばらつきは今回除外）。共有の描画関数を `S._meshNest` フラグでゲートし、旧巣には非干渉（mesh生成時のみ true。production の旧巣はフラグ未定義＝従来描画）。

- **涙型・床フラット**: `meshLensShape()` の `floorFlat` を各種別で引き上げ（base 0.5→0.6、nursery 0.55→0.64 等、clamp上限 0.7→0.78）。`buildNodeBlobAndSlots()` で mesh時のみ天井側(dy<0)を×1.14でドーム状に持ち上げ＝涙型。
- **有機的な不規則輪郭**: `buildNodeBlobAndSlots()` で mesh時のみ blobセグメント 12→14、半径ジッターを ±15%→±25%（`rr = r*(0.66+rand*0.5)`）。
- **ラッパ口**: `buildEdgeStamps()` で mesh時のみ、トンネルが部屋（shaft/entrance以外）につながる端のスタンプ半径を口元ほど拡大（`flareLen=baseR*3.4`、最大×1.85）。
- 検証: プレビューが hidden 固着＋ブラウザキャッシュで一度旧コードを読んだため、`?cb=<ts>` でキャッシュバスト後に eval 確認 → `S._meshNest=true`、blob 14セグ、スタンプ半径 4→11（flare比 2.5）、76ノード/85エッジ、例外0。**スクショは hidden 固着で未取得のため、見た目はユーザーが githack で確認予定**。

### 追記: プレビュー検証は Claude in Chrome（実ブラウザ）が有効

headless プレビュー（`mcp__Claude_Preview__*`）はタブが `visibilityState:hidden` に固着すると rAF が止まり描画されず、スクショもタイムアウトする。**Claude in Chrome MCP（`mcp__Claude_in_Chrome__*`）でローカルChromeを操作すれば描画される**（CDPの `computer{screenshot}` はバックグラウンドでもフレームを強制描画）。手順: `select_browser` → `navigate('http://localhost:8765/?dev=1&cb=<ts>')`（キャッシュバスト）→ `javascript_tool` で `window._dbg.generateMeshNest()`＋モーダル除去＋`S.cam` 直接フィット → `computer{action:'screenshot'}`。中身は `n.invEgg/invLarva/invFood` を直接 seed して D描画（ブルードの山）を確認できる。

### 追記: A/Bの中間（画像1寄り）— 部屋を大きく＆少なく＆密着

参考画像（実物の巣）と比較し「部屋もっと大きく／中身／テクスチャ」の指摘。空の部屋を seed で埋めるとクリーム色のブルードで画像1に急接近＝**中身が最大要因（実プレイでは自動で埋まる）**を実機確認。構造はユーザー選択で A(蜂の巣状ぎっしり)とB(スケッチの蛇行トンネル)の**中間**へ。

- `meshRoomRadius()` を全体的に拡大（nursery 34→46、food 31→42、queen 45→48×1.5 等）。
- `roomsPerBand 11→8`（大きい部屋を少なめに）、`thinWall dp(12)→dp(8)`（薄壁で密着）、`bandGap dp(35)→dp(24)`（層も詰める）。蛇行トンネル＋Y字は維持。
- 検証（実Chrome、`localhost:8765`）: 52ノード/61エッジ、全連結、Y字6、重なり4ペア（大きい部屋＋薄壁で微増）。中身 seed＋カラーモードOFFで、大きいクリーム色のブルード室が画像1に接近。緑＝食料色(#4ade80, ゲーム仕様)は画像1と乖離（種色への変更は別途検討）。

### 追記: Y字ジャンクションの不具合修正（浮いて見える/無意味な二股/ゴチャつき）

実機ズームで「シャフトが部屋につながって見えない（独立空洞っぽいジャンクション）」「二股が結局同じ方向の近い部屋に割れて無意味＋ゴチャつく」との指摘。

- `forkConnect()`: 二股は「2ターゲットがsrcから見て角度差 ≥0.55rad かつ 互いに ≥dp(80) 離れている」場合だけ作る。不成立なら最寄り1本に（無意味な二股を作らない）。junction半径 dp(9)→dp(5)（トンネル径）＋位置を src 寄り(0.4)に置いて独立空洞に見せない。
- `addE`: 短い枝（length < dp(70)＝ジャンクションの脚など）は蛇行を×0.42に抑えてスッキリ（長い主シャフトは蛇行維持）。
- 検証（実Chrome）: ジャンクション 6→4個・全て degree3（意味のある二股のみ）。女王部屋の出口直下で小さく分岐し左右の別部屋（育児/食料）へ繋がる様子・短い枝が直線的になったのをスクショで確認。57ノード、例外0。

### 追記: 同サイズで密度UP＋横幅を活かす（実機フィードバック）

部屋の大きさは維持したまま「もう少し密に＋左右の空きを埋めたい」との指摘。実Chromeで値をライブテストして確定:
- `roomsPerBand 8→12`（同サイズで数を増やして密に）、`widthUse 0.72→0.93`・`sideMargin dp(150)→dp(110)`（横幅を活かす）、`bandGap dp(24)→dp(16)`（層も詰める）。
- 検証（実Chrome）: 71部屋・横幅使用 約73→約81パーセント・重なり 4ペアのみ。部屋14＋thinWall7まで攻めると重なり15に増えたため、12部屋/thinWall8で密度と重なりのバランスを取った。

## 2026-06-17 巣断面のリアル化(E): トンネルの壁陰影

A+B+Dの続き。トンネルを「掘った土の管」に見せる壁陰影を追加。

- `drawStaticNestSoilLayer()` の cavGr 塗りの後に、トンネル(エッジのスタンプ)へ2パスのバッチ描画を重ねる: ①全幅をなだらかに暗く(`rgba(64,44,27,0.32)`, rScale 1.0)＝管の凹み感、②中央にごく淡い光(`rgba(214,194,162,0.16)`, rScale 0.68)＝床のわずかな照り。どちらも `drawStamps()` 1回ずつで静的キャッシュに焼くので毎フレーム無料。
- 初回実装では中央リッジが濃すぎ(0.55/0.52)てスタンプごとに明点が出る「数珠つなぎ」感が出たため、実機確認(`localhost:8765`)で不透明度を下げ半径を広げて(0.16/0.68)滑らかな管に調整。
- 検証: ヘッドレスプレビューが hidden 固着でスクショ不可だったため、ユーザーの実ブラウザ(`localhost:8765`)でビーズ感解消を目視確認。コードは全て上限付き `forEach`＋`drawStamps` でハング構造なし、`edgeFrac`→count も有限を eval で確認。残り(F 構造のハニカム化)は未着手。
...
- **横長楕円レイアウト**: `RADIAL_CONST` に `ASPECT_X 1.46 / ASPECT_Y 0.60` を追加し、円座標を横長楕円へ写像（位置のみ伸縮、ノードは丸のまま）。キャンバスが横長(約2480×940・比~2.6)になり、横長モーダルの幅を使い切る＝フィット倍率が上がりノードが大きく表示(デスクトップで約0.5→0.72)。
- **余白・間隔を拡大**: SPOKE_R0 250→300、RING_STEP 104→120、ROW_SEP 64→78、QUEEN_R0/RSTEP も拡大。
- **ラベルをスポーク外端へ**: 枝ラベルを中心寄り(重なりの原因)から最外ノードの外側(`SPOKE_R0+(maxD+0.9)*RING_STEP`)に移動。ノードと重ならない。
- リングは `<ellipse>` 描画に変更。拡大倍率クランプを `0.28–1.3` → `0.62–1.5`(読みやすさ優先、はみ出しはモーダルスクロール)。
- 検証(preview): 楕円アスペクト2.64・61ノードが x455–2207 / y147–817 に分散・7楕円リング・8ラベル、`data-research-buy` で brood_1 購入0→1・radial維持・エラー0。実機の見た目は要確認(スクショ当環境タイムアウト)。
...
- **歩行(三脚歩行)**: 脚のストライドを実移動量(`pose`差分→`a._px/_py`)に連動。静止個体は脚も止まる(ムーンウォーク防止)。2つの三脚群(前左/中右/後左 ↔ 鏡)を逆位相(`phA`/`phB`)で運ぶ。
- **毛づくろい**: 静止・非運搬・非休憩の個体が、個体ごとにずれた周期(`a.anim`シード)でたまに立ち止まり、前脚を頭へ持ち上げて拭う＋触角を速く震わせる。
- **触角**: 移動中はより活発に揺れる。
- **相互作用**: 毎フレームO(n)の空間グリッド前処理(`_contactMap`、≤700体・sprite/dot時はスキップ)で「近接かつ向かい合う」ペアを周期ゲートで抽出。片方の触角を相手へ伸ばして触れ合い＋小さな光、どちらかが食料運搬中なら緑の口移し(栄養交換)玉を表示。
- すべて描画のみ(ゲーム状態は変更しない。位置は `a.x/a.y`、向きは `a.rot`/`a._drawRot` を参照)。
- 検証(preview): リロード後ループ稼働(S.t進行)・460体すべてvector描画・コンソールエラー0を確認。見た目はスクショ当環境タイムアウトのため実機確認に委ねる。

## 2026-06-15 クッキー再設計・ゴミ病気・放射状ツリー・表示調整（複数フェーズ）

遠征/研究/防衛更新の続き。確定判断: オフラインクッキーは研究(自動発酵・熟成)だけ例外/クッキー室は発見率+1%/室/病気は常時ゲージ＋1体毎分。蟻の自然アニメは今回見送り。

### Phase A — 表示調整・襲撃加速の撤回・クッキー文言

- 食料表示: `fmtFood(n)` 追加(10万未満=整数、10万以上=`fmtCount`のK/M)。`v-food` を `fmtFood(food)/fmtFood(capsFood)` に。
- **`fmtCount` の既存バグ修正**: 末尾0除去の正規表現 `/\.?0+$/` が整数 `"150"`→`"15"` のように整数部の0まで削っていた(150000が"15K"表示)。小数点を含むときだけ削るよう修正。研究コスト等の全数値表示も正しくK/M表記に。
- 遠征中の襲撃加速を撤回: `getExpeditionRaidAccel()` を常に1へ。レイド予兆減算を等速 `dt` に戻す。遠征モーダル警告は「守りが手薄」のみ(「襲撃が早まる」は削除)。
- 初回クッキー発見画面の説明に「クッキーは巣を開いている間に集まる(研究で一部はお留守番中も)/クッキー室で見つけやすく」を追記。
- 検証(preview): 食料 90000→"90000"・150000→"150K"・cap200000→"200K"・1.5M→"1.5M"、5000/12000は素の数値を確認。

### Phase B — クッキー室を「発見率+1%/室」に

- `getWorkerCookieChance()` の倍率方式(`cookieRoomMul = 1.5^N`)を撤廃し、加算方式に: `cookieRoomAdd = cookieRoomCount × COOKIE_ROOM_FIND_ADD(0.01)` を base 項に加算（甘味探索×2等とも相乗）。
- 定数 `COOKIE_ROOM_FIND_ADD = 0.01` 追加。`COOKIE_ROOM_MUL_PER_ROOM`/`S.cookieRoomMul` は互換のため残置（find計算では不使用）。
- ドック説明「発見率×1.5/室」→「クッキー発見率+1%/室」、ツリップを「+1%/室（現在 +N%・開いている間）」に、建設時つぶやきも更新。
- 検証(preview): ドック説明の更新、cookieRoomCount操作、コンソールエラー0を確認（チャンス式は加算の1行変更）。

### Phase C — ゴミ過剰で病気（個体減少）＋ゴミ総量ゲージ

- 既存の `wasteRatio = larvaWasteTotal/(larvaTotal×WASTE_FULL_PER_LARVA)` を `S._wasteRatio` に保存して流用。
- 定数 `DISEASE_WASTE_ON=1.0`/`DISEASE_WASTE_OFF=0.7`(ヒステリシス)/`DISEASE_LOSS_INTERVAL=60`/`DISEASE_LOSS_COUNT=1`。
- `tickDisease(dt)`(update内で毎フレーム): `wr≥ON`で病気発症(警告トースト＋女王つぶやき)、`<OFF`で回復。発症中は60秒ごとに `killSickAnt()`(働きアリ優先で1体減・golden再クランプ)＋`アリ-1`fx＋shake＋トースト(ゴミ室が無ければ「ゴミ室が必要」)。
- HUD: トップバーに「ゴミ」ボックス(`#hygiene-box`/`v-hygiene`)を常時追加。`updateUI`で `S._wasteRatio` を%表示＋緑→黄→橙→(病気)赤に色分け、病気時は🦠＋赤枠。
- 解消にはゴミ室＋清掃で総量を下げる(ゴミ室は搬出量+1＆近接先になり清掃が進む)。
- 検証(preview): ゲージ 30%緑/80%橙/病気100%赤＋🦠 を確認。損耗ループはrAF停止のため実時間未確認だが既存の toast/fx/recalc/getRoomCache 流用で実装。

### Phase D — 放射状（蜘蛛の巣）研究ツリー（拡大モーダルのみ）

- 前回先送りの放射状ツリーを実装。拡大モーダルの `renderResearchTreeModal` を横レーン詰め込みから `buildRadialResearchInner`(新規)に差し替え。パネル内ミニツリーは現状の縦レーン維持。
- `computeRadialResearchLayout()`: 女王枝を中心ハブ周りの内側リング(深さ別)に、他8枝を放射スポーク(角度45°等分、深さ＝中心距離、サブ行＝スポーク垂直オフセット)に配置。全ノードの極座標→直交を計算し中心寄せ。
- ノード描画は既存 `renderResearchTreeNode(def,cx,cy,accent)` を再利用(`.rtree-node` の translate(-50%,-24px) によりleft/top=ドット中心＝極座標と一致)。prereqリンクは中心同士を結ぶ `<line>`。装飾として同心円(rings)＋放射の糸(threads)、中心に👸女王ハブ、各スポーク端に枝ラベル。
- CSS追加: `.rtree-radial`/`.rtree-web-ring`/`.rtree-web-thread`/`.rtree-queen-hub`/`.rtree-spoke-label`。倍率は最小0.28まで許可(収まらない時はモーダルスクロール)。
- 検証(preview): 拡大モーダルで radial 描画(ノード61・女王ハブ1・スポークラベル8・リング7)、座標が38×34バケットに分散(重なり無し)、`data-research-buy` 委譲で gather_1 を購入0→1、購入後も radial 維持、コンソールエラー0。※スクショは当環境でタイムアウトのため構造検証。
