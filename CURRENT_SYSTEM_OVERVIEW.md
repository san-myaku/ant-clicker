...
- Result modal shows the enemy-kind tally + 戦死 count + rally note + 波 count. (The post-defeat recovery buff was removed by request — defeat is just the loss now.) Cadence/interval stays owned by `getNextRaidIntervalSec` / `RAID_INTERVAL_*` (Codex's pacing). v3: seasonal-predator bosses.
