...
- **Not yet done** (later phases per the plan): F4 still needs live tuning of growth pacing/band progression, and F5 adopting the mesh format as the real save/generation format and removing the dev toggle and old generator. Until F5, the production generator (`expandMap`/`initWorld`) and normal save format are **unchanged**.
