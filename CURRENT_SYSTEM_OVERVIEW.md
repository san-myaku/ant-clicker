...
### Mesh Nest Generator Prototype (dev-only, not production)

A second, experimental nest generator is being built per `NEST_STRUCTURE_REDESIGN_PLAN.md` to replace the tree-shaped generator above with a looped mesh (multiple routes between chambers, no single central trunk, banded chamber clusters connected by short tunnels). This is **F1 of a multi-phase plan** and is currently **dev-only**; it does not affect production saves or the normal generator.

- `generateMeshNest(userOpts)` — builds a brand-new `S.full` (nodes/edges) from scratch: entrance + queen room, then for each `S.band` layer places a dense cluster of rooms (`MESH_GEN_DEFAULTS.roomsPerBand`) using tight radius-based spacing on both axes (`rowStep = maxRad * 2.0` vertically, `colStep = maxRad * 2.6` horizontally, each row centered on the entrance x and zig-zag offset by half a column on odd rows — not stretched across the full band height/world width), links rooms within a band for intra-band loops, adds multiple descent shafts from the queen straight to band-0 (no single trunk), and adds multiple inter-band tunnels per band boundary (loop-forming, not one-parent-per-room). Reuses existing blob/edge-stamp/band rendering as-is (`buildNodeBlobAndSlots`, `buildEdgeStamps`, `drawStaticNestSoilLayer`, static visual cache). After building, it resets runtime arrays (ants/enemies/etc. — acceptable for a one-off preview tool), calls `ensureMainShaftState`/`rebuildAllEdgeStamps`/`buildBackgroundAssets`/`updateCameraLimits`, and re-fits the camera (`fitWorldToView(0.95, true)`).
- Tunnel length is kept short by construction: edge-length median measured around 125-135 (dp units) after the horizontal-spacing fix, down from ~360 when columns were stretched across the full world width (the same "stretch to fill" mistake the row-spacing fix already corrected vertically, initially missed on the horizontal axis). Tighter packing also increases loop count, since more room pairs fall within the intra-band link distance threshold.
- `meshLensShape(n)` / `meshRoomRadius(type)` — give chambers a lens/teardrop silhouette (wide, flat floor, domed ceiling) and per-type sizing (queen largest, waste smallest), per the plan's room-shape spec.
- Reachable only via dev mode: a new row in the dev-tools panel (`巣生成(試作):` label, button `id="btn-dev-mesh-nest"`) calls `generateMeshNest()` directly; also exposed at `window._dbg.generateMeshNest` for console use. Dev mode itself is unlocked via `?dev=1` or the existing long-press gesture; this button and row are hidden until then.
- Verified (headless Playwright, `localhost:8765`): 25 consecutive generator runs with zero exceptions (43–49 nodes, 44–60 edges each run); BFS spanning-tree check confirms real loops (e.g. one run: 46 nodes/51 edges → 6 independent cycles); spawned ants ran 8s of live gameplay on a generated mesh with zero console errors; screenshots confirm queen → multiple early descents → distinct banded clusters → loop tunnels, with existing soil/bowl/tunnel-wall rendering intact.
- Known limitation carried over from the existing camera system, not introduced by this work: `S.cam.minScale`'s fit-to-width bias (tuned for tall trunk-style nests) means `fitWorldToView` may not zoom out enough to show a full short-and-wide mesh nest in one frame. Not changed as part of this prototype.
- **Not yet done** (later phases per the plan): F2 visual/shape polish (thinner shared walls, shorter tunnels, room-shape tuning), F3 loop-aware ant pathfinding (current ant movement/tasks still assume tree-style parent-edge traversal — has not been exercised against a real mesh under sustained play), F4 builder AI / nest-growth rules adapted to the new topology, F5 adopting the mesh format as the real save/generation format and removing the dev toggle and old generator. Until F5, the production generator (`expandMap`/`initWorld`) and save format are **unchanged**.
...
- **"Living nest" vector animation** (vector LOD / near camera only): each vector ant's legs use a movement-aware **alternating tripod gait** — stride amplitude/frequency scale with the ant's actual per-frame speed (derived from `pose` delta vs cached `a._px/a._py`), so stationary ants stop "moonwalking"; the two tripods (front-L/mid-R/rear-L vs the mirror) move in opposite phase (`phA`/`phB`). Idle, non-carrying ants occasionally **groom** (`grooming`, a per-ant time-window via `a.anim` seed): they pause, sweep the front legs up to the head, and the antennae quiver faster. Antennae are livelier while moving. **Interaction pass** (`_contactMap`): a per-frame O(n) spatial-grid pre-pass over the vector ant list (capped at ≤700, skipped in sprite/dot modes) finds near, mutually-facing pairs on a periodic gate; matched ants reach one antenna toward each other with a small contact spark, and if either carries food a green **trophallaxis** (口移し) droplet shows between them. All cosmetic; no gameplay/state changes (positions read from `a.x/a.y`, `a.rot`).
...
- **Disease from overflowing garbage** (`tickDisease`): the larva-room waste ratio is also stored as `S._wasteRatio` each tick and shown as a top-bar ゴミ gauge (`#hygiene-box`/`v-hygiene`, green→yellow→orange→red, 🦠 when sick). When `S._wasteRatio ≥ DISEASE_WASTE_ON (1.0)` disease starts (`S._disease.active`, warning toast + queen whisper); while active, every `DISEASE_LOSS_INTERVAL (60)`s `killSickAnt()` removes `DISEASE_LOSS_COUNT (1)` ant (worker-first, then other roles, golden re-clamped) with a `アリ -1` fx/shake/toast. It clears when the ratio drops below `DISEASE_WASTE_OFF (0.7)` (hysteresis). Building a waste room + cleaning is the fix (waste rooms add haul + nearer drop, so the ratio falls). Live-loop only; not applied offline.
...
- UI: the 兵種編成 section lives in the dedicated **soldier panel** (`#modal-soldier`, opened by the always-visible ⚔️ `#btn-soldier`; `renderSoldierModal` shows soldier count / defense power / next cost + `renderCasteSection` with ± steppers `adjustCasteRatio`, scout+heavy ≤ 100 + per-caste descriptions), shown only when `casteSystem` is unlocked. (Previously this was inside the expedition modal.)

### 13.x Expedition system (遠征)

The expedition feature lets the colony dispatch ants to the outside world for delayed rewards. Opened from the **always-visible** top-bar 🏕️ button (`#btn-expedition`, placed directly in `#top-bar` next to ⚔️/⚙ so it isn't collapsed into the mobile hamburger menu → `#modal-expedition`), gated by `isExpeditionUnlocked()` = the `expedition` research branch flag (see section 13 for the Lv8+250-worker auto-unlock). Starting an expedition spawns a departure animation: `spawnExpedWalkers()` sends 4–18 little ants (workers + soldiers at the back) walking out from the nest entrance (`_expedWalkers`, drawn world-space by `drawExpedWalkers`, updated by `updateExpedWalkers`; cosmetic, not saved).
...
- **Risk**: dispatched soldiers are physically away (subtracted from `G.ants.soldier`), so the colony's defense is genuinely weaker while an expedition is out — the modal shows a "兵隊が外に出ています…守りが手薄" banner. (The earlier raid-rate acceleration was removed per user request: `getExpeditionRaidAccel()` now always returns 1; raids do **not** come sooner during expeditions.)
- **Party composition (遠征枝研究)**: `exped_party`「遠征編成」(flag `expedParty`) unlocks a per-route 派遣規模 stepper (×1/1.5/2/3, transient `_expedScale`, not saved). Higher scale multiplies the required worker/soldier counts (`getExpedParty`) and raises success (`+15%/×`, cap +20%). `exped_lore` adds flat `expedSuccess` (+4%/Lv, via `getResearchBonusRaw`); `exped_supply` multiplies returned rewards (`expedReward`, +20%/Lv). Each launch **locks** its `successRate` and `heavyRatio` into the record at dispatch (`getExpeditionSuccessRate(def, party)`), so editing the caste mix while away doesn't change the in-flight odds. Failure worker losses are reduced by the dispatched heavy-caste ratio.
...
- **モグラ (v3 underground boss #3)**: a rare boss-tier invader (real ecology: moles are subterranean predators of ants/brood). Stored in `S.invaders` with `isBoss:true` (so it inherits the pests' soldier-pathing + tap targeting), gated by `MOLE_MIN_ROOMS 6` / `MOLE_MIN_DEPTH 4` / `MOLE_MIN_WINS 1` on a `S.moleTimer` (`MOLE_SPAWN_MIN 240`–`480`s, `MOLE_CHANCE 0.6`), one at a time. `hp = MOLE_HP_BASE 48 + soldiers × MOLE_HP_PER_SOLDIER 2.2` (scales with the army → ~flat kill-time). **Two phases**: `spawnMole()` spawns it **offset in the soil** (`MOLE_DIG_OFFSET 210`dp from the target room, queen-away side) in `phase:'dig'`; it tunnels toward the room at `MOLE_DIG_SPEED 34`dp/s kicking up `dust()` (土埃) + faint ground-rumble shake, then on arrival switches to `phase:'raid'` (resets `lifeT`), bursts dust + "侵入！" toast, and the fight begins (the timeout clock only runs in `raid`, so the dig is prep time). **Combat (soldiers + tap)**: soldiers reuse `hunt_invader` to reach the room then switch to a new `engage_mole` task (cluster on it; idle soldiers prefer it 50%); damage is applied centrally in `applyMoleSoldierDPS` (both phases) by counting soldiers within `dp(MOLE_ENGAGE_RADIUS 72)` × `MOLE_SOLDIER_DPS 1.6 × dt`; taps do `MOLE_TAP_DMG 3` (`hitInvader` `isBoss` branch). **Threat** (`moleEat`, every `MOLE_DMG_INTERVAL 2.4`s): eats `MOLE_EAT_ANTS 3` workers + `MOLE_EAT_EGGS 5` eggs + `MOLE_EAT_LARVAE 1` larva (+room food). **Resolve** (`updateMoleInvader` → splice): driven off (hp≤0) → `+MOLE_REWARD_FOOD 260`/`+MOLE_REWARD_COOKIE 22` + whisper; **timeout** (`lifeT ≥ MOLE_DURATION 28`s) → final `MOLE_ESCAPE_EAT_MUL 3`× chomp **+ `collapseMoleRoom(room)`**: the target room's held eggs/larvae/food/cookie are subtracted from `G` (lost), inventory zeroed, `type→'rubble'` (`collapsed=true`, excluded from `isCountedRoomType` → no caps/count), `G.recalc()`+`normalizeInventories()`. The room is destroyed but **passage is kept** (no node/edge removal → nest never strands); `collapseMoleRoom` hard-guards `queen`/`entrance`/`shaft` and `spawnMole` never targets `queen`/`rubble` (a regression that collapsed the queen room crashed worker-AI was fixed). Rendered by `drawMoleInvader` (realistic side-profile: velvety cylindrical body, pink pointed snout+whiskers, large spade digging claws that scratch harder while digging, soil mound, boss HP bar + raid-only time-left gauge) and `drawCollapsedRoom` (dirt-filled rubble overlay). Runtime-only (`S.invaders` not serialized; `rubble` *type* persists if saved but `collapsed` flag doesn't). Debug: dev-mode `🦫モグラ` button + `window.__forceMole()`. Tunables: `MOLE_*`. Next v3: 梅雨の浸水 (flood disaster).
