...
- `world.band` を保存/復元する
- `band` が無い旧セーブはロード時に可視ノードから掘削帯を再構築する
- 建築アリAIは掘削対象選択前に `ensureBandReady()` で無効な掘削帯を補修する

## 2026-05-29 追記: スマホUI

- 959px以下ではトップバーを食料・クッキー・個体数・⚙の1段表示にする
- スマホの⚙メニューにはセーブ、エクスポート、日記、目標、全体表示、エラー、描画設定を集約する
- スマホの操作パネルはボトムシート方式。折りたたみ時はハンドル、成長バー、TAPボタンのみ常時表示する
- ハンドルタップで強化パネル、蟻/部屋/研究タブ、購入UIを展開する
- 960px以上のPC表示では既存ボタンを元の場所に戻し、従来レイアウトを維持する
