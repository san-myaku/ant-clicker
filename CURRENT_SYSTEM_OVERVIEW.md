...
- `generateMeshNest(userOpts)` — builds a brand-new `S.full` (nodes/edges) from scratch: entrance + queen room, then for each `S.band` layer places a dense cluster of rooms (`MESH_GEN_DEFAULTS.roomsPerBand`) using tight radius-based row spacing (`rowStep = maxRad * 2.0`, not stretched across the full band height), links rooms within a band for intra-band loops, adds multiple descent shafts from the queen straight to band-0 (no single trunk), and adds multiple inter-band tunnels per band boundary (loop-forming, not one-parent-per-room). Reuses existing blob/edge-stamp/band rendering as-is (`buildNodeBlobAndSlots`, `buildEdgeStamps`, `drawStaticNestSoilLayer`, static visual cache). After building, it resets runtime arrays (ants/enemies/etc. — acceptable for a one-off preview tool), calls `ensureMainShaftState`/`rebuildAllEdgeStamps`/`buildBackgroundAssets`/`updateCameraLimits`, and re-fits the camera (`fitWorldToView(0.95, true)`).
