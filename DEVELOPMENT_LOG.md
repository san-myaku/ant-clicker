...
- 残作業（未着手）: F2（薄壁の密着・トンネル短縮・ラッパ口などの見た目の詰め）、F3（蟻の経路探索のループ対応＝最短/混雑回避）、F4（建築AI・成長ロジックの新トポロジー対応）、F5（セーブ新形式への正式採用とdevトグル撤去）。現状は旧ジェネレーター(`expandMap`/`initWorld`)が本番のまま完全に維持されている。
